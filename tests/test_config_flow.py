...
async def test_user_flow_success(hass: HomeAssistant, mock_validate):
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"], {CONF_HOST: "1.2.3.4", CONF_DEVICE_ID: "0AeLoJZc"}
    )
    assert result["type"] is FlowResultType.CREATE_ENTRY
    assert result["data"] == {CONF_HOST: "1.2.3.4", CONF_DEVICE_ID: "0AeLoJZc"}
    assert result["result"].unique_id == "0AeLoJZc"
...
async def test_user_flow_duplicate_aborts(hass: HomeAssistant, mock_validate):
    MockConfigEntry(
        domain=DOMAIN, unique_id="0AeLoJZc",
        data={CONF_HOST: "x", CONF_DEVICE_ID: "0AeLoJZc"},
    ).add_to_hass(hass)
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": SOURCE_USER}
    )
...
def test_device_id_from_name():
    assert device_id_from_name("GLAMP_0AeLoJZc._ged7._tcp.local.") == "0AeLoJZc"
    assert device_id_from_name("glamp_0AeLoJZc._ged7._tcp.local.") == "0AeLoJZc"
...
def _discovery() -> ZeroconfServiceInfo:
    ...
    return ZeroconfServiceInfo(
        ...
        port=80,
        hostname="GLAMP_0AeLoJZc.local.",
        type="_ged7._tcp.local.",
        name="GLAMP_0AeLoJZc._ged7._tcp.local.",
        properties={"mac": "50D21301C7B2", "model": "GLAMP001"},
    )
...
async def test_zeroconf_flow_success(hass: HomeAssistant, mock_validate):
    ...
    assert result["type"] is FlowResultType.CREATE_ENTRY
    assert result["data"][CONF_DEVICE_ID] == "0AeLoJZc"
    assert result["data"][CONF_MAC] == "50D21301C7B2"
    assert result["result"].unique_id == "0AeLoJZc"
